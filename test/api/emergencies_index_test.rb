...
class EmergenciesIndexTest < ActionDispatch::IntegrationTest
  ...
  test 'GET /api/v1/emergencies/ should return all emergencies when some emergencies exist' do
    ...
    assert_equal(
      json_response['emergencies'][0]['code'],
      'E-00000001'
    )
    assert_equal(
      json_response['emergencies'][0]['fire_severity'],
      0
    )
    assert_equal(
      json_response['emergencies'][0]['police_severity'],
      1
    )
    assert_equal(
      json_response['emergencies'][0]['medical_severity'],
      2
    )
  end
  ...
  def setup_resolved_emergencies
    post '/api/v1/responders/', responder: {
      type: 'Fire',
      name: 'F-100',
      capacity: 1
    }
